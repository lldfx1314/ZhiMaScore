...
public class ScoreTrend extends View {
    ...
    private int[] score = new int[]{60, 63, 69, 78, 82, 89};
    ...
    private float medScoreYCoordinate;
...
//  阴影画笔
    ...
    private void initData() {
        ...
        float minScoreYCoordinate = viewHeight * 0.55f;
        medScoreYCoordinate = viewHeight * ((0.55f-0.15f)/2+0.15f);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 绘制0对应的最下面的一条线
        drawDottedLine(canvas, viewWith * 0.15f, viewHeight * 0.55f, viewWith, viewHeight * 0.55f);
        // 绘制0到100的纵坐标文字和横坐标对应的文字
        drawText(canvas);
        // 绘制横坐标的线以及横坐标上的刻度
        ...
        drawPath(canvas);
        // 绘制中间线
        drawDottedLine(canvas, viewWith * 0.15f, medScoreYCoordinate, viewWith, medScoreYCoordinate);
        // 绘制折线
    ...
    private boolean validateTouch(float x, float y) {
        ...
        //月份触摸区域
        //计算每个月份X坐标的中心点
        float monthTouchY = viewHeight * 0.6f - dipToPx(3);//减去dipToPx(3)增大触摸面积
...
//                canvas.drawLine(0,viewHeight * 0.6f, viewWith, viewHeight * 0.6f, straightPaint);
                brokenPath.lineTo(scorePoints.get(i).x, viewHeight * 0.55f);
                brokenPath.lineTo(scorePoints.get(0).x, viewHeight * 0.55f);
                brokenPath.lineTo(scorePoints.get(0).x, scorePoints.get(0).y);
                brokenPath.close();
            }
        }
//  初始化渐变色
        int[] shadeColors = new int[]{Color.argb(255, 28, 177, 226), Color.argb(255, 28, 177, 226)};
        //  渐变阴影
        Shader mShader = new LinearGradient(0, 0, 0, getHeight(), shadeColors, null, Shader.TileMode.CLAMP);
        mPaintShader.setShader(mShader);

        //  绘制渐变阴影
        canvas.drawPath(brokenPath, mPaintShader);
    ...
    private void drawMonthLine(Canvas canvas) {
        ...
        for (int i = 0; i < monthCount; i++) {
            ...
            canvas.drawLine(coordinateX, viewHeight * 0.6f, coordinateX, viewHeight * 0.6f + dipToPx(4), straightPaint);
        }
    }
    ...
    private void drawText(Canvas canvas) {
        ...
        canvas.drawText(String.valueOf(50), viewWith * 0.1f - dipToPx(10), medScoreYCoordinate + textSize * 0.25f, textPaint);
        canvas.drawText(String.valueOf(minScore), viewWith * 0.1f - dipToPx(10), viewHeight * 0.55f + textSize * 0.25f, textPaint);
        ...
        for (int i = 0; i < monthText.length; i++) {
            ...
            if (i == selectMonth - 1) {
                ...
                r2.top = viewHeight * 0.6f + dipToPx(4) + textSize / 2;
                r2.right = coordinateX + textSize + dipToPx(4);
                r2.bottom = viewHeight * 0.6f + dipToPx(4) + textSize + dipToPx(8);
                canvas.drawRoundRect(r2, 10, 10, textPaint);

            }
            //绘制月份
            canvas.drawText(monthText[i], coordinateX, viewHeight * 0.6f + dipToPx(4) + textSize + dipToPx(5), textPaint);
